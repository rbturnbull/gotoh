...
class GotohCountsTest(unittest.TestCase):
    ...
    def levenshtein(self, string1, string2):
        ...
        score = g[1] + g[2] + g[3]
        print(
            "Testing '%s' and '%s' compared with Levenshtein Distance [%d == %d]"
            % (string1, string2, score, d)
        )
        self.assertAlmostEqual(score, d)
    ...
    def identical_strings(self, string, match=0, mismatch=-1, gap=-1, extension=-1):
        print(
            "Testing '%s' against itself with parameters (%s, %s, %s, %s)"
            % (string, match, mismatch, gap, extension)
        )
        g = self.counts(string, string, match, mismatch, gap, extension)

        self.assertAlmostEqualGotoh(g, (len(string), 0, 0, 0), delta=1.0)
    ...
    def replacement(self, string, char, match=0, mismatch=-1, gap=-1, extension=-1):
        ...
        g = self.counts(string, replacement_string, match, mismatch, gap, extension)
        print("counts returned:", g)
        self.assertAlmostEqualGotoh(g, (len(string) - n, n, 0, 0), delta=0.5)
